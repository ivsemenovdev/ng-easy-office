...
1. Пользователь выбирает `.docx` на странице `/diagnostic-import` **или папку** с несколькими `.docx`.
2. `POST /api/diagnostic/parse` — парсинг через **pizzip** (для папки — по одному запросу на файл).
3. Пользователь выбирает регион и больницу, нажимает **«Добавить в БД»** или **«Сохранить все в БД»**.
4. `POST /api/diagnostic/acts` — запись в `diagnostic_acts`.
5. Просмотр сохранённых актов — страница `/hospital-acts`.

**Групповая загрузка:** input с атрибутом `webkitdirectory` — браузер отдаёт все файлы из папки и подпапок; на клиенте фильтруются только `.docx`, затем последовательный parse/save через существующие эндпоинты.
...
| 2026-06-21 | 0.3 | Групповая загрузка `.docx` из папки на `/diagnostic-import` |
